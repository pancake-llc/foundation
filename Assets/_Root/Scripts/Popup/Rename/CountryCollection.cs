...
namespace Pancake.SceneFlow
{
    ...
    [Serializable]
    [CreateAssetMenu(menuName = "Pancake/Game/Country Collection")]
    public class CountryCollection : ScriptableObject
    {
        [SerializeField] private CountryData[] collection;

        public CountryData[] Collection => collection;
        ...
        private void Refresh()
        {
            ...
            collection = new CountryData[241];
            string[] guids = UnityEditor.AssetDatabase.FindAssets("t:texture2D", new[] {@"Assets\_Root\Sprites\StarterPack\CountryIcons"});
            for (var i = 0; i < 241; i++)
            {
                collection[i] = new CountryData() {code = (ECountryCode) i, name = countriesName[i]};
