...
namespace Pancake.SaveData
{
    public static class IO
    {
        ...
        internal const string BACKUP_FILE_SUFFIX = ".bac";
        internal const string TEMPORARY_FILE_SUFFIX = ".tmp";
        ...
        public static bool DirectoryExists(string directoryPath) { return  System.IO.Directory.Exists(directoryPath); }
        ...
        public static string GetDirectoryPath(string path, char seperator = '/')
        {
            ...
            if (slash == -1) Console.LogError("Path provided is not a directory path as it contains no slashes.");
        ...
        {
            if (DirectoryExists(directoryPath))  System.IO.Directory.Delete(directoryPath, true);
        }

        public static string[] GetFiles(string path, bool getFullPaths = true)
        {
            var paths =  System.IO.Directory.GetFiles(path);
            if (!getFullPaths)
            {
                for (int i = 0; i < paths.Length; i++)
        ...
        public static byte[] ReadAllBytes(string path) { return File.ReadAllBytes(path); }

        public static void WriteAllBytes(string path, byte[] bytes) { File.WriteAllBytes(path, bytes); }

        public static void CommitBackup(MetaData metadata)
        {
            Console.Log("Committing backup for " + metadata.path + " to archive location " + metadata.Location);

            var temporaryFilePath = metadata.FullPath + TEMPORARY_FILE_SUFFIX;

            if (metadata.Location == ELocation.File)
            {
                var oldFileBackup = metadata.FullPath + TEMPORARY_FILE_SUFFIX + ".bak";

                // If there's existing save data to overwrite ...
                if (FileExists(metadata.FullPath))
                {
                    // Delete any old backups.
                    DeleteFile(oldFileBackup);
                    // Rename the old file so we can restore it if it fails.
                    MoveFile(metadata.FullPath, oldFileBackup);

                    try
                    {
                        // Now rename the temporary file to the name of the save file.
                        MoveFile(temporaryFilePath, metadata.FullPath);
                    }
                    catch (Exception)
                    {
                        ...
                        MoveFile(oldFileBackup, metadata.FullPath);
                        throw;
                    }
                ...
                else MoveFile(temporaryFilePath, metadata.FullPath);
            }
            else if (metadata.Location == ELocation.PlayerPrefs)
            {
                PlayerPrefs.SetString(metadata.FullPath, PlayerPrefs.GetString(temporaryFilePath));
