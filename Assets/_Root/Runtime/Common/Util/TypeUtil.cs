...
namespace Pancake
{
    public static class TypeUtil
    {
        ...
        private static readonly Assembly AssembUnity = typeof(UnityEngine.Object).Assembly;
        private static readonly Assembly[] PreferredAssemblies;
        ...
        static TypeUtil()
        {
            using (var lst = TempCollection.GetList<Assembly>())
            {
                ...
                PreferredAssemblies = lst.ToArray();
            }
        }
        ...
        {
            foreach (var assemb in PreferredAssemblies)
            {
            ...
            foreach (var assemb in AppDomain.CurrentDomain.GetAssemblies().Except(PreferredAssemblies))
            {
                foreach (var tp in assemb.DefinedTypes)
                {
                    yield return tp;
        ...
        public static IEnumerable<Type> GetTypesAssignableFrom(System.Type rootType) { return GetTypes(t => rootType.IsAssignableFrom(t)); }
        ...
        {
            var assemb = (from a in System.AppDomain.CurrentDomain.GetAssemblies() where a.GetName().Name == assembName || a.FullName == assembName select a)
                .FirstOrDefault();
            if (assemb != null)
            {
                return (from t in assemb.GetTypes() where t.FullName == typeName select t).FirstOrDefault();
            }
            else
            {
                return null;
        ...
        private static System.Type obsoleteAttribType = typeof(System.ObsoleteAttribute);
        public static bool IsObsolete(this System.Reflection.MemberInfo member) { return Attribute.IsDefined(member, obsoleteAttribType); }
    }
}