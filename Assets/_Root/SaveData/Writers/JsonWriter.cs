...
namespace Pancake.SaveData
{
	internal class JsonWriter : Writer
	{
		...
		public JsonWriter(Stream stream, MetaData metadata) : this(stream, metadata, true, true){}

		internal JsonWriter(Stream stream, MetaData metadata, bool writeHeaderAndFooter, bool mergeKeys) : base(metadata, writeHeaderAndFooter, mergeKeys)
		{
		...
		{ 
			StartWriteProperty(name); baseWriter.Write(metadata.encoding.GetString(value, 0, value.Length)); EndWriteProperty(name);
		}

		internal sealed override void StartWriteFile()
		{
            if (writeHeaderAndFooter) baseWriter.Write('{');
            base.StartWriteFile();
		}
		...
		internal override void StartWriteProperty(string name)
		{
            ...
            if(metadata.prettyPrint) baseWriter.Write(' ');
            baseWriter.Write(':');
            if (metadata.prettyPrint) baseWriter.Write(' ');
        }
