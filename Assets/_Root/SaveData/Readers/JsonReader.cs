...
{
    /*
     * 	Specific Reader for reading JSON data.
     * 
    ...
    public class JsonReader : Reader
    {
        ...
        internal JsonReader(Stream stream, MetaData metadata, bool readHeaderAndFooter = true)
            : base(metadata, readHeaderAndFooter)
        {
         ...
         * 	Can be used in conjunction with Read(CustomType) to read a property.
         */
        public override string ReadPropertyName()
        {
            ...
            Console.Log("<b>" + propertyName + "</b> (reading property)", null, serializationDepth);
        ...
        protected override Type ReadKeyPrefix(bool ignoreType = false)
        {
            ...
            if (propertyName == CustomType.TYPE_FIELD_NAME)
            {
                ...
                dataType = ignoreType ? null : Reflection.GetType(typeString);
                propertyName = ReadPropertyName();
            }

            if (propertyName != "value")
                throw new FormatException("This data is not Key Value data. Expected property name \"value\", found \"" + propertyName + "\".");

            return dataType;
        }

        protected override void ReadKeySuffix() { EndReadObject(); }
        ...
        internal override bool StartReadDictionary() { return StartReadObject(); }
        ...
        internal override void EndReadDictionaryKey() { ReadCharIgnoreWhitespace(':'); }
        ...
        internal override bool EndReadDictionaryValue()
        {
            ...
            if (c != ',') throw new FormatException("Expected ',' seperating Dictionary items or '}' terminating Dictionary, found '" + c + "'.");
            return false;
        }


        internal override bool StartReadCollection() { return ReadNullOrCharIgnoreWhitespace('['); }
        ...
        internal override bool EndReadCollectionItem()
        {
            ...
            if (c == ']') return true;
            // Else, we should expect a comma.
            if (c != ',') throw new FormatException("Expected ',' seperating collection items or ']' terminating collection, found '" + c + "'.");
            return false;
        }
        ...
        internal override byte[] ReadElement(bool skip = false)
        {
            ...
            StreamWriter writer = skip ? null : new StreamWriter(new MemoryStream(metadata.bufferSize));
        ...
        private static bool IsOpeningBrace(char c) { return (c == '{' || c == '['); }

        private static bool IsEndOfValue(char c)
        {
            return (c == '}' || c == ' ' || c == '\t' || c == ']' || c == ',' || c == ':' || c == END_OF_STREAM_CHAR || c == '\n' || c == '\r');
        }

        private static bool IsTerminator(char c) { return (c == '}' || c == ']'); }

        private static bool IsQuotationMark(char c) { return c == '\"' || c == '“' || c == '”'; }

        private static bool IsEndOfStream(char c) { return c == END_OF_STREAM_CHAR; }
        ...
        public override void Dispose() { baseReader.Dispose(); }
    }
}