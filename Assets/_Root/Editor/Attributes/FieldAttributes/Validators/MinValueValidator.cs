        ...
        public override void Validate(SerializedField element)
        {
            switch (element.serializedProperty.propertyType)
            {
                case SerializedPropertyType.Integer:
                    if (value != null)
                        element.serializedProperty.intValue = Mathf.Max(element.serializedProperty.intValue, value.intValue);
                    else
                        element.serializedProperty.intValue = Mathf.Max(element.serializedProperty.intValue, System.Convert.ToInt32(attribute.value));
                    break;
                case SerializedPropertyType.Float:
                    if (value != null)
                        element.serializedProperty.floatValue = Mathf.Max(element.serializedProperty.floatValue, value.floatValue);
                    else
                        element.serializedProperty.floatValue = Mathf.Max(element.serializedProperty.floatValue, attribute.value);
                    break;
            }
        }
