...
namespace Pancake.Editor.SaveData
{
    internal class EditorStyle
    {
        ...
        public EditorStyle()
        {
            ...
            link = new GUIStyle();
            link.fontSize = 16;
            if (EditorGUIUtility.isProSkin) link.normal.textColor = new Color(0.262f, 0.670f, 0.788f);
            else link.normal.textColor = new Color(0.129f, 0.129f, 0.8f);

            toggle = new GUIStyle(EditorStyles.toggle) {stretchWidth = false};
        }
    }
}