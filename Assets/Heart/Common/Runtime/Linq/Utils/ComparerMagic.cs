...
namespace Pancake.Linq
{
    ...
    internal sealed class ComparerReverser<T> : IComparer<T>
    {
        ...
        public ComparerReverser(IComparer<T> wrappedComparer) { this._wrappedComparer = wrappedComparer; }
#if !(UNITY_4 || UNITY_5)
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        public int Compare(T x, T y) { return _wrappedComparer.Compare(y, x); }
    }
    ...
    internal static class ComparerExtensions
    {
        ...
        public static IComparer<T> Reverse<T>(this IComparer<T> comparer) { return new ComparerReverser<T>(comparer); }
    }
    ...
    internal sealed class LambdaComparer<T, TU> : IComparer<T>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        public int Compare(T x, T y) { return _comparer.Compare(_selector(x), _selector(y)); }
    }
    ...
    internal sealed class ReverseLambdaComparer<T, TU> : IComparer<T>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
#endif
        public int Compare(T x, T y) { return _comparer.Compare(_selector(y), _selector(x)); }
    }
}