...
namespace Pancake.UI
{
    public sealed class CompositeLifecycleEvent<TLifecycleEvent>
    {
        ...
        public int? FindNextPriority(int? priority)
        {
            ...
            if (!priority.HasValue) return orderedPriorities[0];
            var index = orderedPriorities.IndexOf(priority.Value);
            if (index == -1) throw new InvalidOperationException($"The priority {priority} does not exist.");
            if (index == orderedPriorities.Count - 1) return null;
            return orderedPriorities[index + 1];
        }

        public IEnumerable<TLifecycleEvent> GetItems(int priority)
        {
            if (!_priorityToLifecycleEvent.ContainsKey(priority)) return Enumerable.Empty<TLifecycleEvent>();
            return _priorityToLifecycleEvent[priority];
        }

        public void AddItem(TLifecycleEvent item, int priority)
        {
            if (!_priorityToLifecycleEvent.ContainsKey(priority)) _priorityToLifecycleEvent[priority] = new List<TLifecycleEvent>();
