...
namespace Pancake.UI
{
    public static class PageContainerExtensions
    {
        ...
        {
            var callbackReceiver = new AnonymousPageContainerCallbackReceiver(onBeforePush, onAfterPush, onBeforePop, onAfterPop);
        ...
        public static void AddCallbackReceiver(
            this PageContainer self,
            Page page,
            Action<Page> onBeforePush = null,
            Action<Page> onAfterPush = null,
            Action<Page> onBeforePop = null,
            Action<Page> onAfterPop = null)
        {
            var callbackReceiver = new AnonymousPageContainerCallbackReceiver();
            callbackReceiver.OnBeforePush += x =>
            {
                var (enterPage, exitPage) = x;
                if (enterPage.Equals(page)) onBeforePush?.Invoke(exitPage);
            };
            callbackReceiver.OnAfterPush += x =>
            {
                var (enterPage, exitPage) = x;
                if (enterPage.Equals(page)) onAfterPush?.Invoke(exitPage);
            };
            callbackReceiver.OnBeforePop += x =>
            {
                var (enterPage, exitPage) = x;
                if (exitPage.Equals(page)) onBeforePop?.Invoke(enterPage);
            };
            callbackReceiver.OnAfterPop += x =>
            {
                var (enterPage, exitPage) = x;
                if (exitPage.Equals(page)) onAfterPop?.Invoke(enterPage);
            };

            var gameObj = self.gameObject;
            if (!gameObj.TryGetComponent<MonoBehaviourDestroyedTracker>(out var tracker))
            {
                tracker = gameObj.AddComponent<MonoBehaviourDestroyedTracker>();
            }

            tracker.Callback += () => self.RemoveCallbackReceiver(callbackReceiver);
