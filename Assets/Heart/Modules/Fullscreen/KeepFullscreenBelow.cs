...
namespace FullscreenEditor
{
    internal static class KeepFullscreenBelow
    {
        ...
        {
            var ordered = Types.ContainerWindow.GetPropertyValue<ScriptableObject[]>("windows").Reverse();

            var missing = Resources.FindObjectsOfTypeAll(Types.ContainerWindow).Select(cw => cw as ScriptableObject);

            return ordered.Concat(missing).Distinct();
        }
