...
namespace PancakeEditor.Scriptable
{
    public class WizardWindow : WindowBase
    {
        ...
        public new static WizardWindow Show() => GetWindow<WizardWindow>("Scriptable Wizard");
        ...
        private void DrawCategoryAsButtons()
        {
            ...
            for (int i = 0; i < categories.Count; i++)
            {
                ...
                GUI.backgroundColor = isSelected ? Uniform.RaisinBlack.Lighten(.5f) : Color.gray;
        ...
        private void DrawUtilityButtons()
        {
            ...
            if (GUILayout.Button(buttonContent, buttonStyle, GUILayout.MaxHeight(ScriptableEditorSetting.BUTTON_HEIGHT)))
            {
                var isDeleted = PancakeEditor.Common.Editor.DeleteObjectWithConfirmation(scriptableBase);
                if (isDeleted)
                {
                    scriptableBase = null;
                    OnTabSelected(_currentType, true);
        ...
        private void Refresh(ScriptableType type)
        {
            switch (type)
            {
                ...
                case ScriptableType.Variable:
                    var variables = ProjectDatabase.FindAll<ScriptableBase>(currentFolderPath);
                    variables.RemoveAll(x => x is not IGuid);
                    _scriptableObjects = variables;
                    break;
