...
namespace Pancake.DebugView
{
    public class PropertyListDebugPageBase<TTargetClass> : DefaultDebugPageBase
    {
        ...
        private readonly Dictionary<string, ButtonCellModel> _buttonCellModels = new Dictionary<string, ButtonCellModel>();
        ...
        public virtual Dictionary<string, SubPageInfo> SubPageTargetPropertyNameToInfoMap { get; } = new Dictionary<string, SubPageInfo>();
        ...
        private IReadOnlyDictionary<string, PropertyInfo> GetPropertyInfos()
        {
            ...
            return propertyInfos.Where(propertyInfo => propertyInfo.GetCustomAttribute<ObsoleteAttribute>() == null).ToDictionary(x => x.Name);
        }
        ...
        protected virtual void Update()
        {
            ...
            {
                if (!_buttonCellModels.TryGetValue(targetPropertyName, out var cellModel) || !_propertyInfos.TryGetValue(targetPropertyName, out var propertyInfo))
                    continue;
        ...
        private void ReloadAllItems(string searchString)
        {
            ...
            foreach (var propertyInfo in _propertyInfos.Values)
            {
                ...
                if (!string.IsNullOrEmpty(searchString) && propertyName.IndexOf(searchString, StringComparison.OrdinalIgnoreCase) == -1)
                    continue;
                ...
                else
                {
                    ...
                    buttonCellModel.Clicked += () =>
                    {
                        ...
                        DebugSheet.Instance.BalloonButton.Show(cellRectTrans,
                            "Copy",
                            () => { GUIUtility.systemCopyBuffer = $"{buttonCellModel.CellTexts.Text}: {buttonCellModel.CellTexts.SubText}"; });
                    };
        ...
        protected virtual bool TryGetOverridePropertyDescription(string propertyName, object value, out string description)
        {
            description = null;
            return false;
        }
