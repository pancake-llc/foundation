...
namespace Pancake.DebugViewEditor
{
    ...
    public sealed class StatefulDrawerEditor : DrawerEditor
    {
        ...
        protected override void OnEnable()
        {
            ...
            _minProgressProp = serializedObject.FindProperty("minProgress");
            _useMiddleStateProp = serializedObject.FindProperty("useMiddleState");
            _middleProgressProp = serializedObject.FindProperty("middleProgress");
            _maxProgressProp = serializedObject.FindProperty("maxProgress");
        }
        ...
        protected override void DrawProperties()
        {
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                using (var ccs = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (ccs.changed)
                    {
                        _minProgressProp.floatValue = Mathf.Min(_minProgressProp.floatValue, _maxProgressProp.floatValue);
                        if (_useMiddleStateProp.boolValue)
                            _minProgressProp.floatValue = Mathf.Min(_minProgressProp.floatValue, _middleProgressProp.floatValue);
                    }
                }
            }
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                ...
                using (var ccs = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (ccs.changed)
                    {
                        _middleProgressProp.floatValue = Mathf.Max(_minProgressProp.floatValue, _middleProgressProp.floatValue);
                        _middleProgressProp.floatValue = Mathf.Min(_middleProgressProp.floatValue, _maxProgressProp.floatValue);
                    }
                }
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                using (var ccs = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (ccs.changed)
                    {
                        _maxProgressProp.floatValue = Mathf.Max(_minProgressProp.floatValue, _maxProgressProp.floatValue);
                        if (_useMiddleStateProp.boolValue)
                            _maxProgressProp.floatValue = Mathf.Max(_middleProgressProp.floatValue, _maxProgressProp.floatValue);
                    }
                }
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("Set State"))
                {
                    ...
                    foreach (DrawerState state in Enum.GetValues(typeof(DrawerState)))
                        menu.AddItem(new GUIContent(state.ToString()),
                            false,
                            () =>
                            {
