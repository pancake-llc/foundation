        ...
        private static string GenerateMessage(string methodName, Type clientType)
        {
            if(clientType != null)
            {
                foreach(var interfaceType in clientType.GetInterfaces())
                {
                    ...
                    if(InitializerUtility.argumentCountsByIArgsTypeDefinition.TryGetValue(genericTypeDefinition, out _))
					{
                        initializableType = InitializerUtility.GetIInitializableType(interfaceType.GetGenericArguments());
                    ...
                    else if(InitializerUtility.argumentCountsByIInitializableTypeDefinition.TryGetValue(genericTypeDefinition, out _))
					{
