...
namespace Sisus.Init
{
	...
	public abstract class ServiceInitializer<TService, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument> : IServiceInitializer<TService, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument> where TService : class
	{
		...
		{
			Task<TService> task = InitTargetAsync((TFirstArgument)services[0], (TSecondArgument)services[1], (TThirdArgument)services[2], (TFourthArgument)services[3]);
		...
		/// <inheritdoc/>
		public Task<TService> InitTargetAsync(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument) => Task.FromResult(InitTarget(firstArgument, secondArgument, thirdArgument, fourthArgument));
	}
}