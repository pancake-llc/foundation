...
namespace Sisus.Init
{
		...
		where TClient : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument, TSeventhArgument, TEighthArgument, TNinthArgument, TTenthArgument, TEleventhArgument>
	{
		...
		#if UNITY_EDITOR
		private protected override NullGuardResult EvaluateNullGuard() => IsNull(FirstArgument) || IsNull(SecondArgument) || IsNull(ThirdArgument) || IsNull(FourthArgument) || IsNull(FifthArgument) || IsNull(SixthArgument) || IsNull(SeventhArgument) || IsNull(EighthArgument) || IsNull(NinthArgument) || IsNull(TenthArgument) || IsNull(EleventhArgument) ? NullGuardResult.ValueMissing : NullGuardResult.Passed;
