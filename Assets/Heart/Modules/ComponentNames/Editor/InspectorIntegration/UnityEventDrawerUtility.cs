...
namespace Sisus.ComponentNames.EditorOnly
{
    internal static class UnityEventDrawerUtility
    {
        ...
        private static void OverrideNames(Object listenerTarget, ref GenericMenu menu)
        {
            ...
            var gameObject = listenerTarget as GameObject;
            if (gameObject == null)
            {
            ...
            var namesAndOverridesCache = new Dictionary<string, string>();
        ...
        private static bool BuildFunctionSelectDropdownMenuUsingInternalMethod(
            SerializedProperty listenerProperty,
            UnityEventBase dummyEvent,
            Object listenerTarget,
            out GenericMenu menu)
        {
            menu = default;
            ...
            if (parameters.Length != 3 || parameters[0].ParameterType != typeof(Object) || parameters[1].ParameterType != typeof(UnityEventBase) ||
        ...
        private static UnityEventBase GetDummyEvent(SerializedProperty unityEventProperty)
        {
            ...
            var targetObject = unityEventProperty.serializedObject.targetObject;
        ...
        private static Type GetStaticTypeFromProperty(SerializedProperty property)
        {
            ...
            var isReferencingAManagedReferenceFieldProperty = typeof(SerializedProperty).GetProperty("isReferencingAManagedReferenceField",
                BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            var isReferencingAManagedReference = (bool) isReferencingAManagedReferenceFieldProperty.GetValue(property, null);
            if (isReferencingAManagedReference)
            {
                ...
                var objectTypenameMethod = typeof(SerializedProperty).GetMethod("GetFullyQualifiedTypenameForCurrentTypeTreeInternal",
                    BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
                ...
                var fieldPathMethod = typeof(SerializedProperty).GetMethod("GetPropertyPathInCurrentManagedTypeTreeInternal",
                    BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
        ...
        private static IEnumerable<ValidMethodMap> CalculateMethodMap(Object target, Type[] t, bool allowSubclasses)
        {
            ...
            foreach (var componentMethod in componentMethods)
            {
                ...
                if (paramatersMatch)
                {
                    var vmm = new ValidMethodMap {target = target, methodInfo = componentMethod};
        ...
        private static void AddFunctionsForScript(GenericMenu menu, SerializedProperty listener, ValidMethodMap method, string targetName)
        {
            ...
            var isCurrentlySet = listenerTarget == method.target && methodName == method.methodInfo.Name && mode == setMode;
            ...
            menu.AddItem(new GUIContent(path), isCurrentlySet, SetEventFunction, new UnityEventFunction(listener, method.target, method.methodInfo, mode));
        }

        private static PersistentListenerMode GetMode(SerializedProperty mode) { return (PersistentListenerMode) mode.enumValueIndex; }
