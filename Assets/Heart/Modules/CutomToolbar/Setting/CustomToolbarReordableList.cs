...
internal static class CustomToolbarReordableList
{
    ...
    {
        var reorderableList = new ReorderableList(configsList,
            typeof(BaseToolbarElement),
            true,
            false,
            true,
            true);
        ...
        {
            var elements = GetNewObjectsOfType<BaseToolbarElement>().ToArray();

            var menu = new GenericMenu();

            var lastSortingGroup = 0;
    ...
    private static List<T> GetNewObjectsOfType<T>() where T : class, IComparable<T>
    {
        ...
        foreach (var item in toolbarOptions)
        {
        ...
        foreach (var type in Assembly.GetAssembly(typeof(T)).GetTypes().Where(myType => myType.IsClass && !myType.IsAbstract && myType.IsSubclassOf(typeof(T))))
        {
            ...
            if (constructor == null)
            {
                ...
                foreach (var item in constructors)
                {
                    ...
                    var tempParameters = new object[parameters.Length];
