...
namespace Pancake.BakingSheet.Unity
{
    public class JsonSheetRowScriptableObject : SheetRowScriptableObject
    {
        ...
        {
            var settings = new JsonSerializerSettings {Error = JsonSheetSOContractResolver.ErrorHandler, ContractResolver = JsonSheetSOContractResolver.Instance,};
        ...
        {
            if (!typeof(ISheetRow).IsAssignableFrom(type)) return null;

            if (string.IsNullOrEmpty(serializedRow)) return null;
