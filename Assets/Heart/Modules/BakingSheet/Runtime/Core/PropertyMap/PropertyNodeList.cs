...
namespace Pancake.BakingSheet.Internal
{
    public class PropertyNodeList : PropertyNode
    {
        ...
        public override int CalculateDepth() { return _child.CalculateDepth() + (_isVertical ? 0 : 1); }
        ...
        private static void ValueSetter(PropertyNode child, object obj, object key, object value)
        {
            if (obj is IList list && key is int idx)
            {
                ...
                {
                    list.Add(child.ValueType.IsValueType ? Activator.CreateInstance(child.ValueType) : null);
                }
