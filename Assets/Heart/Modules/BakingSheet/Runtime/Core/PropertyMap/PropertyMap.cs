...
namespace Pancake.BakingSheet.Internal
{
    ...
    public partial class PropertyMap
    {
        ...
        {
            int idx = 0;
            int next = path.IndexOf(Config.INDEX_DELIMITER, StringComparison.Ordinal);
            ...
            while (next != -1)
            {
                ...
                idx = next + 1;
                next = path.IndexOf(Config.INDEX_DELIMITER, idx, StringComparison.Ordinal);
            }
        ...
        public PropertyMap(SheetConvertingContext context, Type sheetType)
        {
            ...
            Root = new PropertyNodeObject(null,
                null,
                rowType,
                RootGetter,
                null,
            ...
            if (typeof(ISheetRowArray).IsAssignableFrom(rowType))
            {
                ...
                Arr = new PropertyNodeList(null,
                    null,
                    arrPropertyInfo.PropertyType,
                    PropertyNodeObject.ValueGetter,
                    null,
                    arrPropertyInfo,
                    resolver,
                    0,
        ...
        public void SetValue(ISheetRow row, int vindex, string path, string value, ISheetFormatter formatter)
        {
            ...
            foreach (var subpath in ParseFlattenPath(path))
            {
                if (node == null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            UnityEngine.Debug.LogError("Column name is invalid");
                            _warned.Add(path);
                        }
                ...
                if (node.IsVertical)
                {
                    if (isVertical)
                    {
                        UnityEngine.Debug.LogError("Nested vertical list is not supported");
                        return;
                    }
            ...
            {
                UnityEngine.Debug.LogError("There is multiple value for a non-vertical column");
                return;
            }
            ...
            if (converter == null)
            {
                UnityEngine.Debug.LogError($"No converter registered for type {node.ValueType}");
                return;
            }

            node.SetValue(row, vindex, _indexes.GetEnumerator(), converter.StringToValue(node.ValueType, value, context));
        }
