...
namespace PancakeEditor.BakingSheet
{
    public class SheetReferenceDropdown : AdvancedDropdown
    {
        ...
        public SheetReferenceDropdown(SerializedProperty property, string label, string targetTypeInfo, AdvancedDropdownState state)
            : base(state)
        {
            _property = property;
            _label = label;
        ...
        protected override AdvancedDropdownItem BuildRoot()
        {
            ...
            foreach (var assetGuid in assetGuids)
            {
                ...
                {
                    var dropdownItem = new AdvancedDropdownItem(rowSO.name) {id = rowSO.GetInstanceID(), icon = AssetPreview.GetMiniThumbnail(rowSO)};
