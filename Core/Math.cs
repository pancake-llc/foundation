...
namespace Pancake
{
    public static class Math
    {
        ...
        public static void Mirror(this ref Vector2 v, Vector2 axis) { v = (2 * (Vector2.Dot(v, axis) / Vector2.Dot(axis, axis)) * axis) - v; }

        public static Vector2 Mirror(this Vector2 v, Vector2 axis) { return (2 * (Vector2.Dot(v, axis) / Vector2.Dot(axis, axis)) * axis) - v; }
        ...
        public static float ClampIn180(this float a, bool bUseRadians = false) { return NormalizeAngle(a, bUseRadians); }
        ...
        /// <param name="smoothTime">Approximately the time it will take to reach the target. A smaller value will reach the target faster</param>
        /// <param name="maxSpeed">Optionally allows you to clamp the maximum speed</param>
        public static float SmoothDampAngle(this float current, float target, ref float currentVelocity, float smoothTime, float maxSpeed = Infinity)
        {
            float deltaTime = Time.deltaTime;
        ...
        public static int WrapIndex(WrapMode mode, int value, int max)
        {
            ...
            {
                case WrapMode.Clamp:
                    return Pancake.Math.Clamp(value, 0, max - 1);
                case WrapMode.Loop:
                    return Pancake.Math.Wrap(value, max);
                case WrapMode.PingPong:
                    return (int) Pancake.Math.PingPong(value, max - 1);
                default:
                    return value;
            }
        }
        ...
        public static float Wrap(WrapMode mode, int value, int max, int min = 0)
        {
            ...
            {
                case WrapMode.Clamp:
                    return Pancake.Math.Clamp(value, min, max);
                case WrapMode.Loop:
                    return Pancake.Math.Wrap(value, max, min);
                case WrapMode.PingPong:
                    return (int) Pancake.Math.PingPong(value - min, max - min) + min;
                default:
                    return value;
            }
        }
        ...
        public static float Wrap(WrapMode mode, float value, float max, float min = 0)
        {
            ...
            {
                case WrapMode.Clamp:
                    return Pancake.Math.Clamp(value, min, max - 1);
                case WrapMode.Loop:
                    return Pancake.Math.Wrap(value, max, min);
                case WrapMode.PingPong:
                    return (int) Pancake.Math.PingPong(value - min, max - min - 1) + min;
                default:
                    return value;
            }
        }
