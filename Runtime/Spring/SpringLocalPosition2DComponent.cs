...
namespace Pancake
{
    public class SpringLocalPosition2DComponent : BaseSpringComponent, ISpringTo<Vector2>, INudgeable<Vector2>
    {
        ...
        private IEnumerator<float> IeSpringToTarget(Vector2 target)
        {
            ...
            while (!M.Approximately(Vector2.SqrMagnitude(new Vector2(transform.localPosition.x, transform.localPosition.y) - target), 0))
            {
        ...
        private IEnumerator<float> IeHandleNudge(Vector2 value)
        {
            ...
            while (!M.Approximately(0, Vector2.SqrMagnitude(position - transform.localPosition)))
            {
                transform.localPosition = _spring.Evaluate(Time.deltaTime);
