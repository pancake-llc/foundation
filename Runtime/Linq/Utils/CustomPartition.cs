...
namespace Pancake.Linq
{
    public class EmptyOrderablePartitioner<T> : OrderablePartitioner<T>
    {
        ...
        public override IList<IEnumerator<KeyValuePair<long, T>>> GetOrderablePartitions(int partitionCount) { return new List<IEnumerator<KeyValuePair<long, T>>>(); }


        public override IEnumerable<KeyValuePair<long, T>> GetOrderableDynamicPartitions() { return new List<KeyValuePair<long, T>>(); }
