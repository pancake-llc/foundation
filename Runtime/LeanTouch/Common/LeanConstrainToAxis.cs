...
namespace Lean.Common
{
    ...
    public class LeanConstrainToAxis : MonoBehaviour
    {
        ...
        public Transform RelativeTo { set { relativeTo = value; } get { return relativeTo; } }
        ...
        public float Minimum { set { minimum = value; } get { return minimum; } }

        [SerializeField] private float minimum = -1.0f;

        /// <summary>The maximum position value relative to the current space.</summary>
        public float Maximum { set { maximum = value; } get { return maximum; } }
        ...
        protected virtual void LateUpdate()
        {
            ...
            if ((axis == AxisType.X || axis == AxisType.MinX) && local.x < minimum)
            {
                local.x = minimum;
                set = true;
            }

            if ((axis == AxisType.X || axis == AxisType.MaxX) && local.x > maximum)
            {
                local.x = maximum;
                set = true;
            }

            if ((axis == AxisType.Y || axis == AxisType.MinY) && local.y < minimum)
            {
                local.y = minimum;
                set = true;
            }

            if ((axis == AxisType.Y || axis == AxisType.MaxY) && local.y > maximum)
            {
                local.y = maximum;
                set = true;
            }

            if ((axis == AxisType.Z || axis == AxisType.MinZ) && local.z < minimum)
            {
                local.z = minimum;
                set = true;
            }

            if ((axis == AxisType.Z || axis == AxisType.MaxZ) && local.z > maximum)
            {
                local.z = maximum;
                set = true;
            }
...
namespace Lean.Common.Editor
{
    ...
    public class LeanConstrainToAxis_Editor : CwEditor
    {
        protected override void OnInspector()
        {
            ...
            if (Any(tgts,
                    t => t.Axis == LeanConstrainToAxis.AxisType.X || t.Axis == LeanConstrainToAxis.AxisType.Y || t.Axis == LeanConstrainToAxis.AxisType.Z ||
                         t.Axis == LeanConstrainToAxis.AxisType.MinX || t.Axis == LeanConstrainToAxis.AxisType.MinY || t.Axis == LeanConstrainToAxis.AxisType.MinZ))
            {
                BeginIndent();
            ...
            if (Any(tgts,
                    t => t.Axis == LeanConstrainToAxis.AxisType.X || t.Axis == LeanConstrainToAxis.AxisType.Y || t.Axis == LeanConstrainToAxis.AxisType.Z ||
                         t.Axis == LeanConstrainToAxis.AxisType.MaxX || t.Axis == LeanConstrainToAxis.AxisType.MaxY || t.Axis == LeanConstrainToAxis.AxisType.MaxZ))
            {
                BeginIndent();
