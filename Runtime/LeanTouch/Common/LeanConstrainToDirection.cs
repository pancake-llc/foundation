...
namespace Lean.Common
{
    ...
    public class LeanConstrainToDirection : MonoBehaviour
    {
        ...
        public Vector3 Forward { set { forward = value; } get { return forward; } }
        ...
        public float MinAngle { set { minAngle = value; } get { return minAngle; } }

        [SerializeField] [Range(0.0f, 180.0f)] public float minAngle = 0.0f;

        /// <summary>This allows you to specify the maximum angle delta between the Forward and Direction vectors in degrees.</summary>
        public float MaxAngle { set { maxAngle = value; } get { return maxAngle; } }

        [SerializeField] [Range(0.0f, 180.0f)] public float maxAngle = 90.0f;
        ...
        protected virtual void LateUpdate()
        {
            if (forward != Vector3.zero && direction != Vector3.zero)
            {
                ...
                if (Mathf.Approximately(oldRotation.x, newRotation.x) == false || Mathf.Approximately(oldRotation.y, newRotation.y) == false ||
                    Mathf.Approximately(oldRotation.z, newRotation.z) == false || Mathf.Approximately(oldRotation.w, newRotation.w) == false)
                {
