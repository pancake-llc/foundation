...
namespace CW.Common
{
    ...
    public abstract class CwEditor : UnityEditor.Editor
    {
        ...
        public void GetTargets<T>(out T tgt, out T[] tgts) where T : Object
        {
            tgt = (T) target;
            tgts = System.Array.ConvertAll(targets, item => (T) item);
        }

        public static void BeginData(SerializedObject newData) { datas.Push(newData); }

        public static void BeginData(SerializedProperty newData) { datas.Push(newData); }

        public static void EndData() { datas.Pop(); }
        ...
        protected void Each<T>(T[] tgts, System.Action<T> update, bool dirty = false, bool apply = false, string undo = null) where T : Object
        {
        ...
        protected bool Any<T>(T[] tgts, System.Func<T, bool> check) where T : Object
        {
            foreach (var t in tgts)
            {
                if (check(t) == true)
        ...
        protected bool All<T>(T[] tgts, System.Func<T, bool> check) where T : Object
        {
            foreach (var t in tgts)
            {
                if (check(t) == false)
        ...
        public static bool Button(string text) { return GUILayout.Button(text); }
        ...
        public static void BeginError(bool error = true) { BeginColor(Color.red, error); }

        public static void EndError() { EndColor(); }
        ...
        public static bool DrawMinMax(string propertyPath, float min, float max, string overrideTooltip = null, string overrideText = null)
        {
            ...
            EditorGUILayout.MinMaxSlider(customContent,
                ref value.x,
                ref value.y,
                min,
                max);
        ...
        private static string StripRichText(string s) { return s.Replace("<b>", "").Replace("</b>", ""); }
