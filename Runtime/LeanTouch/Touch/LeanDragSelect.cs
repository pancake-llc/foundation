...
namespace Lean.Touch
{
    ...
    public class LeanDragSelect : MonoBehaviour
    {
        ...
        public LeanSelectByFinger Select { set { select = value; } get { return select; } }

        [SerializeField] private LeanSelectByFinger select;

        /// <summary>If you begin dragging while objects are already selected, skip?</summary>
        public bool RequireNoSelectables { set { requireNoSelectables = value; } get { return requireNoSelectables; } }

        [SerializeField] private bool requireNoSelectables;

        /// <summary>If you begin dragging on a point that isn't above a selectable object, skip?</summary>
        public bool RequireInitialSelection { set { requireInitialSelection = value; } get { return requireInitialSelection; } }

        [SerializeField] private bool requireInitialSelection;

        /// <summary>Automatically deselect all objects when the drag starts?</summary>
        public bool DeselectAllAtStart { set { deselectAllAtStart = value; } get { return deselectAllAtStart; } }

        [SerializeField] private bool deselectAllAtStart;

        /// <summary>Must the next selected object be within a specified world space distance?\n\n0 = Any distance.</summary>
        public float MaximumSeparation { set { maximumSeparation = value; } get { return maximumSeparation; } }

        [SerializeField] private float maximumSeparation;

        [System.NonSerialized] private List<FingerData> fingerDatas;

        [System.NonSerialized] private bool waitingForSelection;
        ...
        private void HandleFingerUp(LeanFinger finger) { LeanFingerData.Remove(fingerDatas, finger); }
        ...
        private void HandleAnySelectedFinger(LeanSelectByFinger select, LeanSelectableByFinger selectable, LeanFinger finger)
        {
            ...
            {
                // Good selection?
                if (maximumSeparation <= 0.0f || fingerData.LastSelectable == null ||
                    Vector3.Distance(fingerData.LastSelectable.transform.position, selectable.transform.position) <= maximumSeparation)
                {
