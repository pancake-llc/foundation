...
namespace Lean.Common
{
    ...
    public struct LeanScreenQuery
    {
        ...
        public LeanScreenQuery(MethodType newMethod)
            : this(newMethod, Physics.DefaultRaycastLayers)
        {
        }
        ...
        public T Query<T>(GameObject gameObject, Vector2 screenPosition)
        {
            ...
            if (TryQuery(gameObject,
                    screenPosition,
                    ref result,
                    ref root,
                    ref worldPosition) == true)
            {
                return result;
            }
            ...
            if (hit != null)
            {
                switch (Search)
                {
                    case SearchType.GetComponent:
                        if (TryGetComponent(hit, ref result, ref component) == false) return false;
                        break;
                    case SearchType.GetComponentInParent:
                        if (TryGetComponentInParent(hit, ref result, ref component) == false) return false;
                        break;
                    case SearchType.GetComponentInChildren:
                        if (TryGetComponentInChildren(hit, ref result, ref component) == false) return false;
                        break;
                }
