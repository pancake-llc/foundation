...
namespace UnityAtoms
{
    ...
    public abstract class AtomEventInstancer<T, E> : MonoBehaviour, IGetEvent, ISetEvent where E : AtomEvent<T>
    {
        ...
        [SerializeField] [ReadOnly] private E _inMemoryCopy;

        /// <summary>
        /// The Event that the in memory copy will be based on when created at runtime.
        /// </summary>
        [SerializeField] private E _base = null;
        ...
        /// <typeparam name="E"></typeparam>
        public void SetEvent<EO>(EO e) where EO : AtomEventBase { throw new Exception($"Event type not reassignable!"); }
        ...
        public void Raise(T item) { Event.Raise(item); }
    }
}
