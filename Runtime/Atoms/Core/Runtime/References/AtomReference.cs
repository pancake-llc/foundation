...
namespace UnityAtoms
{
        ...
        where VI : AtomVariableInstancer<V, P, T, E1, E2, F>
    {
            ...
            get
            {
                switch (_usage)
                {
                    ...
                    case (AtomReferenceUsage.VARIABLE_INSTANCER):
                        return _variableInstancer == null || _variableInstancer.Variable == null ? default(T) : _variableInstancer.Value;
        ...
        /// </summary>
        [SerializeField] private C _constant = default(C);
        ...
        protected AtomReference() { _usage = AtomReferenceUsage.VALUE; }

        protected AtomReference(T value)
            : this()
        {
        ...
        public static implicit operator T(AtomReference<T, P, C, V, E1, E2, F, VI> reference) { return reference.Value; }
        ...
        public E GetEvent<E>() where E : AtomEventBase
        {
            switch (_usage)
            {
                ...
                default:
                    throw new Exception(
                        $"Can't retrieve Event when usages is set to '{AtomReferenceUsage.DisplayName(_usage)}'! Usage needs to be set to '{AtomReferenceUsage.DisplayName(AtomReferenceUsage.VARIABLE)}' or '{AtomReferenceUsage.DisplayName(AtomReferenceUsage.VARIABLE_INSTANCER)}'.");
            }
        }
        ...
        public void SetEvent<E>(E e) where E : AtomEventBase
        {
            switch (_usage)
            {
                ...
                default:
                    throw new Exception(
                        $"Can't set Event when usages is set to '{AtomReferenceUsage.DisplayName(_usage)}'! Usage needs to be set to '{AtomReferenceUsage.DisplayName(AtomReferenceUsage.VARIABLE)}' or '{AtomReferenceUsage.DisplayName(AtomReferenceUsage.VARIABLE_INSTANCER)}'.");
            }
        }
    }
}
