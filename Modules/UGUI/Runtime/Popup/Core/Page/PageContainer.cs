...
namespace Pancake.UI.Popup
{
    ...
    public sealed class PageContainer : GameComponent
    {
        ...
        private readonly List<IPageContainerCallbackReceiver> _callbackReceivers = new List<IPageContainerCallbackReceiver>();
            ...
            string pageId = null)
        {
            ...
            if (!DefaultPopupSetting.EnableInteractionInTransition)
            {
                if (DefaultPopupSetting.ControlInteractionAllContainer)
                {
                    ...
                    foreach (var modalContainer in ModalContainer.Instances) modalContainer.Interactable = false;
            ...
            if (!DefaultPopupSetting.EnableInteractionInTransition)
            {
                if (DefaultPopupSetting.ControlInteractionAllContainer)
                {
                    ...
                    if (Instances.All(x => !x.IsInTransition) && ModalContainer.Instances.All(x => !x.IsInTransition) &&
                        SheetContainer.Instances.All(x => !x.IsInTransition))
                    {
                        foreach (var pageContainer in Instances) pageContainer.Interactable = true;
                        foreach (var modalContainer in ModalContainer.Instances) modalContainer.Interactable = true;
