...
namespace Pancake.UI.Popup
{
    ...
    {
        private static readonly Dictionary<int, ModalContainer> InstanceCacheByTransform = new Dictionary<int, ModalContainer>();
        private static readonly Dictionary<string, ModalContainer> InstanceCacheByName = new Dictionary<string, ModalContainer>();

        [SerializeField] private string displayName;
        [SerializeField] private ModalBackdrop overrideBackdropPrefab;
        ...
        private readonly List<IModalContainerCallbackReceiver> _callbackReceivers = new List<IModalContainerCallbackReceiver>();
        ...
        /// <summary>
        ///     By default, <see cref="IAssetLoader" /> in <see cref="DefaultPopupSetting" /> is used.
        ///     If this property is set, it is used instead.
        /// </summary>
        public IAssetLoader AssetLoader { get => _assetLoader ?? DefaultPopupSetting.AssetLoader; set => _assetLoader = value; }
        ...
        private void Awake()
        {
            ...
            _backdropPrefab = overrideBackdropPrefab ? overrideBackdropPrefab : DefaultPopupSetting.ModalBackdropPrefab;
        ...
        /// <returns></returns>
        public static ModalContainer Of(Transform transform, bool useCache = true) { return Of((RectTransform) transform, useCache); }
        ...
        public void AddCallbackReceiver(IModalContainerCallbackReceiver callbackReceiver) { _callbackReceivers.Add(callbackReceiver); }

        /// <summary>
        ///     Remove a callback receiver.
        /// </summary>
        /// <param name="callbackReceiver"></param>
        public void RemoveCallbackReceiver(IModalContainerCallbackReceiver callbackReceiver) { _callbackReceivers.Remove(callbackReceiver); }
        ...
        {
            return App.StartCoroutine(PushRoutine(typeof(Modal),
                resourceKey,
                playAnimation,
                onLoad,
                loadAsync,
        ...
        public AsyncProcessHandle Push(
            ...
            Action<(string modalId, Modal modal)> onLoad = null)
        {
            return App.StartCoroutine(PushRoutine(modalType,
                resourceKey,
                playAnimation,
                onLoad,
                loadAsync,
        ...
        {
            return App.StartCoroutine(PushRoutine(typeof(TModal),
                resourceKey,
                playAnimation,
            ...
            string modalId = null)
        {
            ...
            if (!DefaultPopupSetting.EnableInteractionInTransition)
            {
                if (DefaultPopupSetting.ControlInteractionAllContainer)
                {
                    foreach (var pageContainer in PageContainer.Instances) pageContainer.Interactable = false;
                    foreach (var modalContainer in Instances) modalContainer.Interactable = false;
            ...
            if (!DefaultPopupSetting.EnableInteractionInTransition)
            {
                if (DefaultPopupSetting.ControlInteractionAllContainer)
                {
                        ...
                        SheetContainer.Instances.All(x => !x.IsInTransition))
                    {
                        ...
                        foreach (var modalContainer in Instances) modalContainer.Interactable = true;
        ...
        private IEnumerator PopRoutine(bool playAnimation, int popCount = 1)
        {
            ...
            if (_orderedModalIds.Count < popCount)
                throw new InvalidOperationException("Cannot transition because the modal count is less than the pop count.");
            ...
            var preprocessHandles = new List<AsyncProcessHandle> {exitModal.BeforeExit(false, enterModal)};
