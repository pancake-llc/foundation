...
namespace Pancake.TagEditor
{
    public static class TagSettingsProvider
    {
        ...
        {
            var provider = new SettingsProvider("Preferences/Pancake/TagSettings", SettingsScope.User)
            {
                label = "Tag",
                ...
                activateHandler = (searchContext, rootElement) =>
                {
                    ...
                    var settings = new SerializedObject(TagSetting.Instance);

                    var title = new Label("Tag Settings")
                    {
                    ...
                    btn.tooltip =
                        "In rare circumstances - such as a bad merge from source control - ScriptableTag's may not have unique identifiers. If ever in doubt, click this to check all assets and note the output in the console.";
        ...
        {
            ScriptableTagRegistry.FindAllAssets<TagGroupBase, ScriptableTag>();
            HashSet<Hash128> set = new HashSet<Hash128>();
            bool hasDuplicates = false;
            for (int i = 0; i < ScriptableTagRegistry.AllAssets.Count; ++i)
            {
                var asset = ScriptableTagRegistry.AllAssets[i].asset;
                if (asset != null && !asset.IsDefault)
                {
                    if (set.Contains(asset.Hash))
                    {
                        ...
                        if (ScriptableTagRegistry.AllAssets[i].group != null)
                            AssetDatabase.SetMainObject(ScriptableTagRegistry.AllAssets[i].group, AssetDatabase.GetAssetPath(asset));
            ...
            if (userInitiated && !hasDuplicates)
                Debug.Log("All assets inheriting from ScritpableTag appear to have unique hashes. Checked " + set.Count + " assets and No Duplicates were detected.");
        }

        private static void UpdateHelpTextLabel(ChangeEvent<string> evt = null)
        {
            switch (TagSetting.Instance.searchMode)
            {
