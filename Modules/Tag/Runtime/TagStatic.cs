...
namespace Pancake.Tag
{
    public static class TagStatic
    {
        ...
        internal static Dictionary<Hash128, HashSet<int>> _AllTaggedGOsByIndex = new Dictionary<Hash128, HashSet<int>>(new Hash128EqualityComparer());

        public static Dictionary<Hash128, HashSet<int>> AllTaggedGOsByIndex
        {
            get
            {
        ...
        /* Experimental WIP*/
        internal static Dictionary<TagQueryGameObject, Action<GameObject>> OnGameObjectStartListeners = new Dictionary<TagQueryGameObject, Action<GameObject>>();
        internal static Dictionary<TagQueryGameObject, Action<GameObject>> OnGameObjectEnabledListeners = new Dictionary<TagQueryGameObject, Action<GameObject>>();
        internal static Dictionary<TagQueryGameObject, Action<GameObject>> OnGameObjectDisabledListeners = new Dictionary<TagQueryGameObject, Action<GameObject>>();
        internal static Dictionary<TagQueryGameObject, Action<GameObject>> OnGameObjectDestroyedListeners = new Dictionary<TagQueryGameObject, Action<GameObject>>();
        ...
        internal static void CheckGlobalQueriesImmediate(Transform t, GOEventType evt)
        {
            ...
            Dictionary<TagQueryGameObject, Action<GameObject>> listeners;
            switch (evt)
            {
                case GOEventType.Disabled:
        ...
        public static void AddListenerOnStart(this TagQueryGameObject query, Action<GameObject> onEnabled)
        {
            HasGlobalListeners = true;
            OnGameObjectStartListeners.Add(query, onEnabled);
        }

        public static void RemoveListenerOnStart(this TagQueryGameObject query) { OnGameObjectStartListeners.Remove(query); }

        public static void AddListenerOnEnabled(this TagQueryGameObject query, Action<GameObject> onEnabled)
        {
            HasGlobalListeners = true;
            OnGameObjectEnabledListeners.Add(query, onEnabled);
        }

        public static void RemoveListenerOnEnabled(this TagQueryGameObject query) { OnGameObjectEnabledListeners.Remove(query); }

        public static void AddListenerOnDisabled(this TagQueryGameObject query, Action<GameObject> onEnabled)
        {
            HasGlobalListeners = true;
            OnGameObjectDisabledListeners.Add(query, onEnabled);
        }

        public static void RemoveListenerOnDisabled(this TagQueryGameObject query) { OnGameObjectDisabledListeners.Remove(query); }

        public static void AddListenerOnDestroyed(this TagQueryGameObject query, Action<GameObject> onEnabled)
        {
            HasGlobalListeners = true;
            OnGameObjectDestroyedListeners.Add(query, onEnabled);
        }

        public static void RemoveListenerOnDestroyed(this TagQueryGameObject query) { OnGameObjectDestroyedListeners.Remove(query); }
        /*-----------------*/
        ...
        internal static void Register(GameObject go, Tag tag)
        {
            ...
            if (!TagStatic._AllTaggedGOsByIndex.ContainsKey(tag.Hash))
            {
                TagStatic._AllTaggedGOsByIndex.Add(tag.Hash, new HashSet<int>());
            }

            // Only store full Tag if they are being found by name via the API
            if (TagStatic.StoreTagNames && !tag.IsDefault && !TagStatic.AllTags.Contains(tag))
            {
                TagStatic.AllTags.Add(tag);
            }

            // Add this gameObject to the list associated with the tag
            int goID = go.GetInstanceID();
            if (!TagStatic._AllTaggedGOs.ContainsKey(goID))
            {
                TagStatic._AllTaggedGOs.Add(goID, go);
            }

            if (!TagStatic._AllGOsTagCount.ContainsKey(goID))
            {
                TagStatic._AllGOsTagCount.Add(goID, 0);
            }

            TagStatic._AllGOsTagCount[goID]++;

            var idxArray = TagStatic._AllTaggedGOsByIndex[tag.Hash];
            if (!idxArray.Contains(goID))
            {
                idxArray.Add(goID);
                TagStatic._AllTaggedGOsByIndex[tag.Hash] = idxArray;
            }
        }
        ...
        internal static void Unregister(GameObject go, Tag tag)
        {
            ...
            if (tag.Hash.IsValid)
            {
                ...
                if (TagStatic._AllGOsTagCount.ContainsKey(goID))
                {
                    if (TagStatic._AllGOsTagCount[goID] > 1)
                    {
                        TagStatic._AllGOsTagCount[goID]--;
                    }
                    else
                    {
                        TagStatic._AllGOsTagCount[goID] = 0;
                        if (TagStatic._AllTaggedGOs.ContainsKey(goID)) TagStatic._AllTaggedGOs.Remove(goID);
                    }
                }

                if (TagStatic._AllTaggedGOsByIndex.ContainsKey(tag.Hash))
                {
                    var goIDHashSet = TagStatic._AllTaggedGOsByIndex[tag.Hash];
                    if (goIDHashSet.Contains(goID))
                    {
                        goIDHashSet.Remove(goID);
                        TagStatic._AllTaggedGOsByIndex[tag.Hash] = goIDHashSet;
                    }
                }

                // Only tracking full Tags if they are being found by name via the API
                if (TagStatic.StoreTagNames && TagStatic.AllTags.Contains(tag))
                {
                    TagStatic.AllTags.Remove(tag);
                }
            }
        }
        ...
        internal static string FindTagNameForHash(Hash128 hash)
        {
            var t = FindTagForHash(hash);
            return t == default ? "untagged" : t.name;
        }

        internal static Tag FindTagForHash(Hash128 hash)
        {
            Tag result = default;
            string[] groupSOGuids = UnityEditor.AssetDatabase.FindAssets("t:" + typeof(ScriptableTagGroup));
        ...
        public class TagQueryGameObject
        {
#if UNITY_EDITOR
            ...
            public TagQueryGameObject() { Reset(); }
        ...
        private static List<GameObject> SingleResult = new List<GameObject>(1);

        public static GameObject GetFirstGameObject(this TagQueryGameObject q)
        {
        ...
        public static List<GameObject> GetGameObjects(this TagQueryGameObject q, bool findAll = true)
        {
        ...
        public static C GetFirstComponent<C>(this TagQueryGameObject q) where C : Component { return GetComponents<C>(q, false).FirstOrDefault(); }

        public static List<C> GetComponents<C>(this TagQueryGameObject q, bool findAll = true) where C : Component
        {
        ...
        private static int searchResultSize = 0;
        private static GameObject[] searchResults = new GameObject[1024];
        private static int mainWorkingResultSize = 0;
        private static int[] mainWorkingResults = new int[1024];

        public static bool GetGameObjects(this TagQueryGameObject q, ref List<GameObject> results, bool findAll = true) =>
            GetGameObjects<GameObject>(q, ref results, findAll);

        public static bool GetGameObjects<T>(this TagQueryGameObject q, ref List<GameObject> results, bool findAll = true) where T : UnityEngine.Object
        {
#if UNITY_EDITOR
        ...
        public static bool GetComponents<C>(this TagQueryGameObject q, ref List<C> results, bool findAll = true) where C : Component
        {
#if UNITY_EDITOR
            if (q.DisableEditorChecks) IgnoreEditorChecks = true;
#endif
...
#if UNITY_EDITOR
            if (!TagSetting.Instance.disableEditorChecks && !IgnoreEditorChecks && !findAll) CheckForMultipleResults(results, q.TagWithSettings);
            IgnoreEditorChecks = false;
#endif
            return (results.Count() > 0);
        }

        public static TagQueryGameObject For(GameObject gameObject)
        {
            TagQueryGameObject q = new TagQueryGameObject();
            q.InitialTargets = new HashSet<GameObject> {gameObject};
            return q;
        }

        public static TagQueryGameObject For(IEnumerable<GameObject> gameObjects)
        {
            TagQueryGameObject q = new TagQueryGameObject();
        ...
        private static TagQueryGameObject AddEntryToQuery(ref TagQueryGameObject q, TagHashWithRules hashWithRule, bool combine = false)
        {
        ...
        private static TagQueryGameObject AddEntriesToQuery(ref TagQueryGameObject q, Tag[] tags, InclusionRule rule, Search search, bool combine = false)
        {
            var settings = q.TagWithSettings;
        ...
        public static TagQueryGameObject Find(TagQuery query)
        {
            TagQueryGameObject q = new TagQueryGameObject();
            return q.Find(query);
        }

        public static TagQueryGameObject Find(this TagQueryGameObject q, TagQuery query, Search search = Search.Target)
        {
        ...
        public static TagQueryGameObject Find(string tag) => Find(ByName(tag));

        public static TagQueryGameObject Find(Tag tag)
        {
            TagQueryGameObject q = new TagQueryGameObject();
        ...
        public static TagQueryGameObject FindAny(IEnumerable<Tag> tags)
        {
            TagQueryGameObject q = new TagQueryGameObject();
            return AddEntriesToQuery(ref q, tags.ToArray(), InclusionRule.Any, Search.Target);
        }

        public static TagQueryGameObject FindAll(IEnumerable<Tag> tags)
        {
            TagQueryGameObject q = new TagQueryGameObject();
            return AddEntriesToQuery(ref q, tags.ToArray(), InclusionRule.MustInclude, Search.Target);
        }

        public static TagQueryGameObject AnyIsFine(this TagQueryGameObject q)
        {
#if UNITY_EDITOR
        ...
        public static TagQueryGameObject ParentWith(this TagQueryGameObject q, Tag tag, Search search = Search.Parent) => With(q, tag, search);
        public static TagQueryGameObject ChildrenWith(this TagQueryGameObject q, Tag tag, Search search = Search.Children) => With(q, tag, search);

        public static TagQueryGameObject With(this TagQueryGameObject q, Tag tag, Search search = Search.Target)
        {
        ...
        public static TagQueryGameObject ParentWithAny(this TagQueryGameObject q, IEnumerable<Tag> tags, Search search = Search.Parent) => WithAny(q, tags, search);
        public static TagQueryGameObject ChildrenWithAny(this TagQueryGameObject q, IEnumerable<Tag> tags, Search search = Search.Children) => WithAny(q, tags, search);

        public static TagQueryGameObject WithAny(this TagQueryGameObject q, IEnumerable<Tag> tags, Search search = Search.Target)
        {
        ...
        public static TagQueryGameObject ParentWithAll(this TagQueryGameObject q, IEnumerable<Tag> tags, Search search = Search.Parent) => WithAll(q, tags, search);
        public static TagQueryGameObject ChildrenWithAll(this TagQueryGameObject q, IEnumerable<Tag> tags, Search search = Search.Children) => WithAll(q, tags, search);

        public static TagQueryGameObject WithAll(this TagQueryGameObject q, IEnumerable<Tag> tags, Search search = Search.Target)
        {
        ...
        public static TagQueryGameObject Without(this TagQueryGameObject q, Tag tag)
        {
            if (tag == null || tag.IsDefault)
            {
                // Clear initial targets as null was passed so nothing should match
        ...
        public static TagQueryGameObject Without(this TagQueryGameObject q, IEnumerable<Tag> tags)
        {
        ...
        #region Tag First GameObject Queries
        ...
        #region Tag First Component Queries
        ...
        #region Tag List<GameObject> Queries
        ...
        #region Tag List<Component> Queries
        ...
        internal static bool TransformMatchesTags(Transform transform, ref TagHashWithRules[] tags, int idx, int count)
        {
            ...
            List<Hash128> tagsInComponents = transform.gameObject.GetComponents<MultiTagGameObject>().SelectMany(x => x.tags).Select(y => y.Hash).ToList();
        ...
        private static HashSet<int> SubSearchResults = new HashSet<int>();
            ...
            if (gameObject != null)
            {
                ...
                for (int t = 0; t < allTransforms.Length; ++t)
                {
                    ...
                    if (found)
                    {
                        ...
                        if (!findAll)
#if UNITY_EDITOR
                            if (IgnoreEditorChecks || TagSetting.Instance.disableEditorChecks || SubSearchResults.Count > 2)
#endif
                                break;
                    }
                }
...
#if UNITY_EDITOR
                if (!TagSetting.Instance.disableEditorChecks && !IgnoreEditorChecks && !findAll && (count + idx == tags.Length))
                {
                    var allGOs = AllTaggedGOs;
            ...
            if (gameObject != null)
            {
                ...
                for (int t = 0; t < allTransforms.Length; ++t)
                {
                    ...
                    if (found)
                    {
                        ...
                        if (!findAll)
#if UNITY_EDITOR
                            if (IgnoreEditorChecks || TagSetting.Instance.disableEditorChecks || results.Count > 2)
#endif
                                break;
                    }
                }
...
#if UNITY_EDITOR
            if (!TagSetting.Instance.disableEditorChecks && !IgnoreEditorChecks && !findAll) CheckForMultipleResults(results, tags);
#endif
            return results;
        }


        private static HashSet<int> WorkingHashSet = new HashSet<int>();
        ...
        internal static int GetTaggedWith<T>(TagHashWithRules[] tags, int idx, int tagCount, bool findAll = true) where T : UnityEngine.Object
        {
            ...
            {
                Hash128 firstTagHash = firstInclusiveTag.hash;
        ...
        public static Hash128 GetHash(Tag tag)
        {
            bool valid = tag != null;
#if UNITY_EDITOR
