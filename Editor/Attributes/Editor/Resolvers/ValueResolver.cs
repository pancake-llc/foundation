...
namespace PancakeEditor.Attribute
{
    public static class ValueResolver
    {
        ...
        public static ValueResolver<string> ResolveString(PropertyDefinition propertyDefinition, string expression)
        {
            if (expression != null && expression.StartsWith("$"))
            {
        ...
        public static bool TryGetErrorString<T>([CanBeNull] ValueResolver<T> resolver, out string error) { return TryGetErrorString<T, T>(resolver, null, out error); }

        public static bool TryGetErrorString<T1, T2>(ValueResolver<T1> resolver1, ValueResolver<T2> resolver2, out string error)
        {
    ...
    public abstract class ValueResolver<T>
    {
        ...
        [PublicAPI]
        public abstract T GetValue(Property property, T defaultValue = default);
    }
    ...
    public sealed class ConstantValueResolver<T> : ValueResolver<T>
    {
        ...
        public ConstantValueResolver(T value) { _value = value; }
        ...
        public override T GetValue(Property property, T defaultValue = default) { return _value; }
    }
}