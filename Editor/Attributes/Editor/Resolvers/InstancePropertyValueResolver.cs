...
namespace Pancake.AttributeDrawer
{
    internal sealed class InstancePropertyValueResolver<T> : ValueResolver<T>
    {
        ...
        public static bool TryResolve(PropertyDefinition propertyDefinition, string expression, out ValueResolver<T> resolver)
        {
            ...
            {
                if (propertyInfo.Name == expression && typeof(T).IsAssignableFrom(propertyInfo.PropertyType) && propertyInfo.CanRead)
                {
        ...
        private InstancePropertyValueResolver(PropertyInfo propertyInfo) { _propertyInfo = propertyInfo; }
