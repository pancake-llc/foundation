...
{
    internal class ReferenceInspectorElement : PropertyCollectionBaseInspectorElement
    {
        ...
        public ReferenceInspectorElement(Property property, Props props = default)
        {
            _property = property;
            _props = props;
        ...
        public override void OnGUI(Rect position)
        {
            ...
            var headerRect = new Rect(position) {height = _skipReferencePickerExtraLine ? 0f : EditorGUIUtility.singleLineHeight,};
            var headerLabelRect = new Rect(position) {height = headerRect.height, width = EditorGUIUtility.labelWidth,};
            ...
            if (_props.inline)
            {
                ...
                {
                    ManagedReferenceGui.DrawTypeSelector(headerRect, _property);
                }
            ...
            {
                AttributeSkin.Foldout(headerLabelRect, _property);

                if (_showReferencePicker)
                {
                    ManagedReferenceGui.DrawTypeSelector(headerFieldRect, _property);
                }
