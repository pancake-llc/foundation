...
namespace PancakeEditor.Attribute
{
    public class HorizontalGroupInspectorElement : PropertyCollectionBaseInspectorElement
    {
        ...
        public HorizontalGroupInspectorElement(float[] sizes = null)
        {
        ...
        public override void OnGUI(Rect position)
        {
            ...
            for (var i = 0; i < ChildrenCount; i++)
            {
                var childWidth = GetChildWidth(i, totalDynamic, dynamicChildCount);
                var child = GetChild(i);
                var childRect = new Rect(position) {width = childWidth, height = child.GetHeight(childWidth), x = position.xMin + xOffset,};
