...
namespace Pancake.AttributeDrawer
{
    public abstract class HeaderGroupBaseInspectorElement : PropertyCollectionBaseInspectorElement
    {
        ...
        protected virtual float GetContentHeight(float width) { return base.GetHeight(width) + InsetTop + InsetBottom; }

        protected virtual void DrawHeader(Rect position) { }

        protected virtual void DrawContent(Rect position) { base.OnGUI(position); }

        public sealed override float GetHeight(float width) { return GetContentHeight(width) + GetHeaderHeight(width); }
