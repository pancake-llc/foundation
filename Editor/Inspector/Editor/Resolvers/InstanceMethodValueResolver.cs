...
namespace Pancake.Editor
{
    internal sealed class InstanceMethodValueResolver<T> : ValueResolver<T>
    {
        ...
        public static bool TryResolve(PropertyDefinition propertyDefinition, string expression, out ValueResolver<T> resolver)
        {
            ...
            {
                if (methodInfo.Name == expression && typeof(T).IsAssignableFrom(methodInfo.ReturnType) && methodInfo.GetParameters() is var parameterInfos &&
        ...
        private InstanceMethodValueResolver(MethodInfo methodInfo) { _methodInfo = methodInfo; }
